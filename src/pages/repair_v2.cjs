...
if (index !== -1) {
    ...
    console.log("SUCCESS: Fixed AdminDashboard imports.");
} else {
    // Try finding by the next line
    const fallbackIndex = content.indexOf("const navigate = useNavigate();");
    if (fallbackIndex !== -1) {
        const rest = content.substring(content.lastIndexOf("export default function", fallbackIndex));
        fs.writeFileSync(path, correctTop + "\n" + rest);
        console.log("SUCCESS: Fixed AdminDashboard imports via fallback.");
    } else {
        console.log("ERROR: Could not find AdminDashboard component.");
    }
}
